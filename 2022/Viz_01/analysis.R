...
month_names <- as.list(substr(month.name, 1, 3)) #need list in gt to map the column name with label
...
all_the_revenue %>%
  ...
  # get country codes from dataset countrypops
  left_join(
  ...
  tab_source_note(
    source_note = md(
      "All reporting shown was created by the **Central DS Team**"
    )) %>%
  tab_footnote(
  ...
  tab_footnote(
    footnote  = md("The `v1.0` build the first version of the offer agent SDK."),
    locations = cells_column_spanners(spanners = "v1.0")
  ) %>%
  tab_footnote(
    footnote  = md("The `v2.0` build added more missions and had an updated offer agent SDK."),
  ...
  tab_options(
    table.background.color        = adjust_luminance("aliceblue", steps = 2),
    heading.background.color      = "aliceblue",
    footnotes.background.color    = adjust_luminance("aliceblue", steps = -0.5),
    source_notes.background.color = adjust_luminance("aliceblue", steps = -0.5),
    table.border.top.width        = px(4),
    table.border.bottom.width     = px(4),
    table.margin.left             = px(4),
    table.margin.right            = px(4),
